...
class CI_DB_oci8_driver extends CI_DB {
    ...
    function procedure($package, $procedure, $params) {
       ...
       // VERIFICA OS CAMPOS DA SP
        foreach ($params as $param) {

            if ($param['type'] == OCI_B_CURSOR) {
                $p_cursor = oci_new_cursor($this->conn_id);
                oci_bind_by_name($stmt, $param['name'], $p_cursor, -1, OCI_B_CURSOR);                
            }elseif ($param['type'] == RNT) {
                oci_bind_by_name($stmt, $param['name'], $retorno, RNT);
            }elseif ($param['type'] == OCI_B_ROWID) {
                oci_bind_by_name($stmt, $param['name'], $retorno, OCI_B_ROWID);
            }else {
                oci_bind_by_name($stmt, $param['name'], $param['value']);
                
            }
        }
        // VERICA SE HÁ UM CURSOR
       $query = ociexecute($stmt);
        
        if ($query == FALSE) {
            $query = oci_error($stmt);
            $erro = 'CÓDIGO: ' . $query['code'] . '<br>';
            $erro .= 'ORACLE: ' . $query['message'] . '<br>';
            $erro .= 'OFFSET: ' . $query['offset'] . '<br>';
            //$erro .= 'SQL: ' . $query['sqltext'] . '<br>';
            //print_r($params);
            //echo "<div class='label label-danger'>".$erro."</div>";
            return FALSE;
        } else {
            // VERIFICA SE EXISTE UM VETOR
            ociexecute($p_cursor, OCI_DEFAULT);            
            if ($retorno == '') {
